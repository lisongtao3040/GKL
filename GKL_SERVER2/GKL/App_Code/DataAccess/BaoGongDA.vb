...
Public Class BaoGongDA
    ...
    Public Function InsListData(ByVal cd As String, ByVal no As String, ByVal user As String, ByVal suu As Integer, ByVal tp_nyu_suu As Integer, ByVal lineid As String) As Boolean
        ...
        sb.AppendLine("           ,case when tuopan_syu_suu=0 then 1 else CEILING(suu/tuopan_syu_suu) end")
    ...
    Public Function InsMSData(ByVal cd As String, ByVal no As String, ByVal user As String, ByVal suu As Integer, ByVal tp_nyu_suu As Integer, ByVal lineid As String, ByVal tp_bar_cd As String) As Boolean
        ...
        Dim msRowCnt As Integer
        If tp_nyu_suu = 0 Then
            msRowCnt = 0
        Else
            msRowCnt = Math.Ceiling(suu / tp_nyu_suu) - 1
        End If

        For i As Integer = 0 To msRowCnt
            ...
            sb.AppendLine("           ,'" & dt.Rows(0).Item("ProductCodeSap") & "/" & dt.Rows(0).Item("ProductCode") & "/" & bg_suu & "/" &
                           CInt(Math.Ceiling(bg_suu / CInt(dt.Rows(0).Item("Package")))) & "/" & dt.Rows(0).Item("localStorage") & "/" & dt.Rows(0).Item("DestinationCode") & "/" & (i + 1) & "/" & dt.Rows(0).Item("ZuoFan") & "'")
            If tp_nyu_suu = 0 Then
                sb.AppendLine("           ," & suu & "")
            Else
                If suu < tp_nyu_suu Then
