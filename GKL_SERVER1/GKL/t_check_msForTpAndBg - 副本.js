...
function localStorageSetItem(key, value){
    ...
    setTimeout(function() {
        WebMethod("api", "SetServerStorage",
                JSON.stringify({
                    login_user_cd: getCookie('login_user_cd'),
                    key: key,
                    value: value
...
//alert(localStorageGetItem('login_user_cd')+'ff');
...
function JoinTP_InsertTrayData() {
    ...
    if ($("#hidTuopanLines").val() == "1" && localStorageGetItem("Active_TpBarcode").Trim() == "") {
        alert("请扫描托盘码");
        return false;
    } else {
...
function SetNewTpBarcode(obj) {
    ...
    if ($("#hidTuopanLines").val() == "1" && newTpBarcode.Trim() == "") {
        ...
        if ($("#defaultTpTd" + idx).text() == '●') {
            localStorageSetItem('Active_TpBarcode', newTpBarcode.Trim());
        }
