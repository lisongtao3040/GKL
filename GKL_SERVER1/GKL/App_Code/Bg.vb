...
Public Class Bg
    ...
    Public Function JoinTP_InsertTrayData(ByVal line_cd As String, ByVal no As String, ByVal cd As String,
        ...
        'If System.Configuration.ConfigurationManager.AppSettings.[Get]("baogong_lines").ToString().IndexOf(line_cd) >= 0 Then
        ...
        '        Dim tp_no As String = "1"
        '        Dim jsonRtvBG As Object = BaoGongFnc(line_cd, cd, no, insUser, insUserName, chk_id, tp_bar_cd, dt, dtRlt, tp_no)

        '        '关联托盘时
        '        Dim jsonRtvTP As Object = Run_InsertTrayData(cd, no, line_cd, dt.Rows(0)("suu").ToString(), tp_no, tp_bar_cd, insUser, insUserName)
        ...
        '        '报工Panel
        '        Dim rtvBgPanel As Object = GetBgPanel(cd, no)
        '        Return JsonConvert.SerializeObject(rtvBgPanel)
    ...
    '报工Panel
    <WebMethod()>
    Public Function GetTpXiangxian(ByVal tps As String) As Object

        Dim TP As New TPVB

        Dim rtv As New List(Of String)


        For Each tp_no As String In tps.Split(",")

            If tp_no.Trim = "" Then
                rtv.Add("")
            Else
                Dim rtvTray As String = TP.GetNoListByTray(tp_no)
                Dim jsonRtv As Object = JsonConvert.DeserializeObject(rtvTray)

                If (rtvTray Is Nothing) OrElse rtvTray = "" Then
                    rtv.Add("×")

                Else
                    Try
                        rtv.Add(jsonRtv("data")("destination").ToString)
    ...
    Public Function WB_InsertTrayData(ByVal line_cd As String, ByVal no As String, ByVal cd As String,
        ...
        Dim tpChk As String = CheckTpList(cd, no, line_cd, tp_bar_cd)
        If tpChk <> "" Then
            Return JsonConvert.SerializeObject(New With {
                            .result = "NG",
                            .msg = "检查托盘时时：" & tpChk("message")
                        })
        End If
    ...
    Public Function Edit_UpdateTrayData(ByVal line_cd As String, ByVal no As String, ByVal cd As String, ByVal insUser As String,
        ...
        '检查托盘向先 便别
        Dim tpChk As String = CheckTpList(cd, no, line_cd, tp_bar_cd2)
        If tpChk <> "" Then
            Return JsonConvert.SerializeObject(New With {
                            .result = "NG",
                            .msg = "检查托盘时时：" & JsonConvert.DeserializeObject(tpChk)("message").ToString()
                        })
        End If
    ...
    Public Function CheckTpList(ByVal cd As String,
                                    ByVal no As String,
                                    ByVal lineid As String,
                                    ByVal tp_bar_cd As String) As String
        ...
        Dim rtvTray As String = TP.GetNoListByTray(tp_bar_cd)
        ...
        Else
            ...
            If (jsonRtv("isSuccess").value) Then
                ...
                'Dim jsonData As Object = JsonConvert.DeserializeObject(jsonRtv("data").value)

                'If jsonRtv("data")("destination").ToString <> dt.Rows(0).Item("DestinationCode") Then
                '    Return JsonConvert.SerializeObject((New With {
                '                .isSuccess = False,
                '                .message = "托盘信息 向先不一致"
                '        }))
                'ElseIf Not (dt.Rows(0).Item("BianCode").ToString.Contains(jsonRtv("data")("belong").ToString) AndAlso jsonRtv("data")("belong").ToString.Trim <> "") Then
    ...
    Public Function InsertTrayData(ByVal cd As String,
        ...
        Dim tpChk As String = CheckTpList(cd, no, lineid, tp_bar_cd)
        If tpChk <> "" Then
            Return tpChk
        End If
        ...
        Dim rtv As String = TP.InsertTrayData(no, formid, qr, tp_bar_cd, user_id, user_name)
