...
public partial class CheckItiran : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Common.NullToEmpty(ViewState["menu_user_cd"]) == "")
            {
            ...
            {
                G_line_id = Request.QueryString["line_id"].ToString();
        ...
        System.Text.StringBuilder csScript = new System.Text.StringBuilder();
        //csScript.AppendLine("window.onload = function(){");
        csScript.AppendLine("   setCookie('login_user_cd', '" + ViewState["menu_user_cd"] + "', 1);");
        //csScript.AppendLine("   alert(getCookie('login_user_cd'));");
        //csScript.AppendLine("};");

        // 在页面响应中注册客户端脚本块
        Page.ClientScript.RegisterStartupScript(this.GetType(), "localStorage", csScript.ToString(), true);
    ...
    private System.Data.DataTable GetMsData(string make_no, string code, bool userDateFlg)
    {
        ...
        return BC.SelTCheckResult(G_line_id, startTime, endTime, make_no, code);
    ...
    public void MsInit(int pageIdx, string make_no, string code, bool userDateFlg)
    {
        ...
        //{
        //    while (dt.Rows.Count > 101)
        //    {
        //        dt.Rows.RemoveAt(101); // 从索引100开始删除（保留前100行）
        //    }
        //}
        //// 如果行数不足100，补充空行
        //else if (dt.Rows.Count < 199)
        //{
        //    int rowsToAdd = 199 - dt.Rows.Count;

        //    for (int i = 0; i < rowsToAdd; i++)
        //    {
        //        DataRow newRow = dt.NewRow();

        //        newRow["chk_no"] = i.ToString() + "_" + i.ToString();


        //        // 如果需要给新行设置默认值，可以在这里设置
        //        // foreach (DataColumn col in dt.Columns)
        //        // {
        //        //     newRow[col] = DBNull.Value; // 或其他默认值
        //        // }
    ...
    public void GetPageData(System.Data.DataTable inDt, int pageIdx)
    {
        ...
        for (i = 0; i <= dt.Rows.Count - 1; i++)
        {
            ...
            //生产明细书
            //gvMs.Rows[i].Attributes["scmx"] =
            //dt.Rows[i]["ProductCodeSap"].ToString()
            //+ "/" + dt.Rows[i]["code"].ToString()
            //+ "/" + dt.Rows[i]["suu"].ToString()
            //+ "/" + dt.Rows[i]["Package"].ToString()
            //+ "/" + dt.Rows[i]["localStorage"].ToString()
            //+ "/" + dt.Rows[i]["DestinationCode"].ToString()
            //+ "/1/" + dt.Rows[i]["make_no"].ToString()
            //;
    ...
    protected void btnInsert_Click(object sender, EventArgs e)
    {
        ...
        TCheckResultBC BC = new TCheckResultBC();
        System.Data.DataTable dt = BC.SelTCheckResult(G_line_id, "", "", make_no, code);
